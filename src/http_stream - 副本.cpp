...
static float get_pixel_mat(cv::Mat m, int x, int y, int c)
{
	assert(x < m.cols && y < m.rows && c < m.channels());
	//return m.data[c*m.rows*m.cols + y*m.cols + x];
	return m.at<Vec3b>(y, x)[c];
}
static void set_pixel_mat(cv::Mat& m, int x, int y, int c, unsigned char val)
{
	if (x < 0 || y < 0 || c < 0 || x >= m.cols || y >= m.rows || c >= m.channels()) return;
	assert(x < m.cols && y < m.h && c < m.c);
	//m.data[c*m.rows*m.cols + y*m.cols + x] = val;
	//printf("\nx = %d, y = %d, c = %d, val = %d", x, y, c, val);
	m.at<Vec3b>(y, x)[c] = val;
	//m.data[c * m.rows * m.cols + y * m.cols + x] = val;
}
void embed_image_replace(cv::Mat source, cv::Mat& dest, int dx, int dy)
{
	int x, y, k;
	int c = 3;
	for (k = 0; k < source.channels(); ++k)
	{
		for (y = 0; y < source.rows; ++y)
		{
			...
			{
				unsigned char val = get_pixel_mat(source, x, y, k);
				set_pixel_mat(dest, dx + x, dy + y, k, val);
			}
		}
    ...
    float jitter, float dhue, float dsat, float dexp)
{
	...
	//namedWindow("crop", 0);
	//namedWindow("out", 0);
	//resizeWindow("crop", 416, 736);
	//resizeWindow("out", 416, 736);
...
#if 0
	//更改为补边的resize方式，代码粘贴过来
	int new_w = w;
	int new_h = h;

	if (((float)w / cropped.cols) < ((float)h / cropped.rows)) {
		new_w = w;
		new_h = (cropped.rows * w) / cropped.cols;
	}
	else {
		new_h = h;
		new_w = (cropped.cols * h) / cropped.rows;
	}

    cv::resize(cropped, sized, cv::Size(new_w, new_h), 0, 0, INTER_LINEAR);
    cv::resize(cropped, sized_tmp, cv::Size(new_w, new_h), 0, 0, INTER_LINEAR);
	if (img.channels() == 3)
	{
		sized = Mat(w, h, CV_8UC3, Scalar(0, 0, 0));
	}
	else
	{
	sized = Mat(w, h, CV_8UC1, 0);
	}

	embed_image_replace(sized_tmp, sized, (w - new_w) / 2, (h - new_h) / 2);
#endif
	//imshow("sized", sized);
	//waitKey(0);
    ...
    if (flip) {
		...
		//imshow("src", sized);
		//imwrite("src.png", sized);
		//imshow("flip", cropped);
		//imwrite("flip.png", cropped);
		//waitKey(0);
        sized = cropped.clone();
    }
    // HSV augmentation
