...
{
	//每前向一个层，state的input参数循环修改为层的output地址，送入l.forward()前向传播函数
    ...
    for(i = 0; i < net.n; ++i)
	{
        ...
        layer l = net.layers[i];
        if(l.delta){
            scal_cpu(l.outputs * l.batch, 0, l.delta, 1);//不进入这个函数进行调试
        }
        l.forward(l, state);
...
float train_network_datum(network net, float *x, float *y)
{
    ...
    *net.seen += net.batch;
    state.index = 0;
    state.net = net;
    state.input = x;//纯图像信息 batch * d.X.cols（416 * 416 * 3）
    state.delta = 0;
    state.truth = y;//标注信息 90 *（4 + 1）
    state.train = 1;
...
float train_network(network net, data d)
{
    ...
    for(i = 0; i < n; ++i)
	{
		//将data d中的训练数据与标签拷贝至net->input，net->truth，至此网络与数据准备完成。);
        ...
        float err = train_network_datum(net, X, y);//训练网路,本次训练共有net.batch张图片
        sum += err;
    }
    free(X);
    free(y);
...
int num_detections(network *net, float thresh)
{
    ...
    for (i = 0; i < net->n; ++i) {
        layer l = net->layers[i];
        if (l.type == YOLO) {
#ifdef DEBUG_TEST //记录在哪个层识别到的框的标志位
...
void fill_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets, int letter)
{
    ...
    for (j = 0; j < net->n; ++j) {
        ...
        if (l.type == YOLO) {
			...
			for (int layerCount = 0; layerCount < count; layerCount++)
			{
				dets[layerCount].from_yolo_layer = j;
			}
#endif
