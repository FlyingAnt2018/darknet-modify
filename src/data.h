...
typedef struct load_args{
    ...
    char **paths;
    char *path;
    int n;//每一个batch训练的时候需要加载的图片总数，是sub的乘积
    int m;//所有训练样本的总数 在train.txt中列出所有样本
    char **labels;
    int h;
    int w;
	int c; // color depth
...
//int setPatchToBigImage(IplImage* pPatch, IplImage* pBackground, CvRect pos, box_label *boxes, int* count);
