...
//从一个char[]中找到图片的名字，不包含后缀
int find_picName(char *str, char *output)
{
	...
	{
		if (buffer[i] == '.' && buffer[i + 1] == 'j' && buffer[i + 2] == 'p' && buffer[i + 3] == 'g')
		{
			iEnd = i;
		}
...
//找文件名 包含后缀
int find_picName_houzhui(char *str, char *output)
{
	...
	int length = 0;
	for (int i = 0; i < iEnd - iStart - 1 + 4; i++)
	{
		output[i] = buffer[iStart + i + 1];
		length++;
	}
...
void replace_image_to_label(char *input_path, char *output_path)
{
    ...
    find_replace(output_path, "/_src_image/", "/labels/", output_path);    // PascalVOC
	find_replace(output_path, "\\_src_image\\", "\\labels\\", output_path);        // COCO

    find_replace(output_path, "/_thirdBatchSample/", "/labels/", output_path);    // PascalVOC
	find_replace(output_path, "\\_thirdBatchSample\\", "\\labels\\", output_path);    // PascalVOC

    find_replace(output_path, "/_makeFullCar/", "/labels_fullCar/", output_path);    // PascalVOC
    find_replace(output_path, "\\_makeFullCar\\", "\\labels_fullCar\\", output_path);    // PascalVOC

    find_replace(output_path, "/_2classesVehicle/", "/labels_2classVehicle/", output_path);    // PascalVOC
    find_replace(output_path, "\\_2classesVehicle\\", "\\labels_2classVehicle\\", output_path);    // PascalVOC
...
void replace_image_to_image(char *input_path, char *output_path)
{
    find_replace(input_path, "/images/train2014/", "/labels/train2014/", output_path);    // COCO
    find_replace(output_path, "\\images\\train2014\\", "\\labels\\train2014\\", output_path);    // COCO

    find_replace(output_path, "/JPEGImages/", "/patch/", output_path);    // PascalVOC
    find_replace(output_path, "\\JPEGImages\\", "\\patch\\", output_path);    // PascalVOC

    find_replace(output_path, "/images/val2014/", "/labels/val2014/", output_path);        // COCO
    find_replace(output_path, "\\images\\val2014\\", "\\labels\\val2014\\", output_path);        // COCO

    find_replace(output_path, "/_src_image/", "/patch/", output_path);    // PascalVOC
	find_replace(output_path, "\\_src_image\\", "\\patch\\", output_path);        // COCO

    find_replace(output_path, "/_thirdBatchSample/", "/patch/", output_path);    // PascalVOC
	find_replace(output_path, "\\_thirdBatchSample\\", "\\patch\\", output_path);    // PascalVOC

    find_replace(output_path, "/_makeFullCar/", "/patch/", output_path);    // PascalVOC
    find_replace(output_path, "\\_makeFullCar\\", "\\patch\\", output_path);    // PascalVOC

    find_replace(output_path, "/_2classesVehicle/", "/patch/", output_path);    // PascalVOC
    find_replace(output_path, "\\_2classesVehicle\\", "\\patch\\", output_path);    // PascalVOC
...
float rand_scale_darkness(float s)
{
    float scale = rand_uniform_strong(0.5, s);
    if(random_gen()%2) return scale;
    return 1./scale;
}
